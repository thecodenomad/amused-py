...
**The first open-source BLE protocol implementation for Muse S headsets (Gen1 & Gen3/Athena)**
...
This library supports multiple Muse S device generations:

- **Muse S Gen 1**: 4 EEG channels (TP9, AF7, AF8, TP10) at up to 250 Hz
- **Muse S Gen 3 (Athena)**: 7 EEG channels (TP9, AF7, AF8, TP10, FPz, AUX_R, AUX_L) at 256 Hz

The library automatically detects your device generation and configures itself accordingly. No code changes needed!

## ⚠️ Important: Gen1 Sleep Monitoring Limitations

**Muse S Gen1 devices are not well-suited for sleep monitoring** due to the following limitations:

- **Limited EEG Channels**: Only 4 channels vs 7 in Gen3, reducing sleep staging accuracy
- **Lower Sampling Rate**: 250 Hz max vs 256 Hz in Gen3, affecting signal quality
- **Stability Issues**: Gen1 may have connection stability issues during long recordings
- **Sleep Presets**: Sleep-optimized presets (p1034, p1035) are designed for Gen3

**For sleep monitoring, we strongly recommend using Muse S Gen3/Athena devices.**

If you must use Gen1 for sleep monitoring:
- Use shorter recording sessions (< 2 hours)
- Apply preset `p1036` for best stability
- Expect lower signal quality and potential connection drops
- Consider Gen1 more suitable for short EEG sessions and research tasks

## Features

- **Multi-Generation Support**: Automatic detection and configuration for Muse S Gen1 and Gen3/Athena devices
- **EEG Streaming**:
  - Gen1: 4 channels (TP9, AF7, AF8, TP10) at up to 250 Hz
  - Gen3: 7 channels (TP9, AF7, AF8, TP10, FPz, AUX_R, AUX_L) at 256 Hz
...
async def stream():
    ...
    # Create streaming client (auto-detects Gen1/Gen3)
...
For Muse S Gen1 devices, the library automatically detects and configures the appropriate settings:

```python
# Explicitly specify Gen1 (optional - auto-detection is enabled by default)
client = MuseStreamClient(
    device_model='gen1',  # or 'gen3', 'auto'
    save_raw=True,
    decode_realtime=True
)

# Gen1-specific features:
# - 4 EEG channels: TP9, AF7, AF8, TP10
# - Max 250 Hz sampling rate
# - Optimized presets: p22, p23, p1036 (lower rate for stability)
# - Quality monitoring and stabilization

# Use lower-rate preset for Gen1 stability
await client.connect_and_stream(
    device.address,
    duration_seconds=30,
    preset='p1036'  # Gen1 optimized preset (128Hz-like)
)
```

### Device Detection

The library automatically detects your Muse generation:

```python
# Auto-detection (recommended)
client = MuseStreamClient(device_model='auto')

# Or explicitly specify
client = MuseStreamClient(device_model='gen1')  # For Gen1 devices
client = MuseStreamClient(device_model='gen3')  # For Gen3/Athena devices
```

### Gen1 vs Gen3 Feature Comparison

| Feature | Muse S Gen1 | Muse S Gen3/Athena |
|---------|-------------|-------------------|
| EEG Channels | 4 (TP9, AF7, AF8, TP10) | 7 (TP9, AF7, AF8, TP10, FPz, AUX_R, AUX_L) |
| Max Sampling Rate | 250 Hz | 256 Hz |
| Recommended Presets | p1036, p23, p22 | p1035, p1034 |
| Quality Monitoring | Built-in stabilization | Standard |
| Visualization | Compatible with 4-channel displays | Full 7-channel support |
| **Sleep Monitoring** | ⚠️ Not recommended | ✅ Fully supported |
...
- **Automatic Gen1/Gen3 detection and configuration**
...
- **Compatible with both Gen1 and Gen3 data formats**
...
- Minimal latency
- **Gen1-specific decoding support**
...
- **Works with both Gen1 and Gen3 recordings**
...
- `p1036`: Gen1 optimized preset (128Hz-like)
...
### Gen1-specific issues?
- Use preset `p1036` for better stability
- Gen1 devices may need longer stabilization time
- Check that device is detected as Gen1 in verbose mode
